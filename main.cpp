...

{
  if (action == GLFW_PRESS) {
	...
	float wHeight = worldTop - WORLD_BOTTOM;
	float wWidth = WORLD_RIGHT - WORLD_LEFT;

	float wx = ((x) / (float)WINDOW_WIDTH) * wWidth;
	float wy = wHeight - ((y) / (float)WINDOW_HEIGHT) * wHeight;
